...
function buildGiveawayEmbed(giveaway: {
  ...
  const endsAtTimestamp = Math.floor(giveaway.endsAt.getTime() / 1000);
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      {
        name: "⏱️ Durée",
        value: `${giveaway.durationMinutes} minute${giveaway.durationMinutes > 1 ? "s" : ""}`,
        inline: true,
      },
      {
...
async function scheduleGiveawayEnd(giveawayId: number, endsAt: Date) {
      ...
      if (giveaway.channelId && giveaway.messageId) {
        try {
          ...
          if (channel && channel.isTextBased()) {
            ...
            if (message) {
              const embed = buildGiveawayEmbed({ ...updated, endsAt: new Date(updated.endsAt) });
              await message.edit({ embeds: [embed] });

              if (selectedWinners.length > 0) {
                await channel.send(
                  `🎊 Félicitations aux gagnants du giveaway **${giveaway.prize}** : ${selectedWinners.map((w) => `**${w}**`).join(", ")} !`
                );
              } else {
                await channel.send(
                  `❌ Le giveaway **${giveaway.prize}** est terminé mais aucun participant n'a été trouvé.`
                );
              }
            }
          }
        } catch (err) {
          console.error("Erreur lors de la mise à jour du message Discord:", err);
        }
      }

      console.log(`✅ Giveaway #${giveawayId} terminé. Gagnants: ${selectedWinners.join(", ") || "Aucun"}`);
    } catch (err) {
      console.error(`❌ Erreur lors de la fin du giveaway #${giveawayId}:`, err);
    }
  }, delay);
...
async function handleEndGiveaway(interaction: ChatInputCommandInteraction) {
  ...
  if (!giveaway) {
    await interaction.editReply("❌ Giveaway introuvable.");
    return;
  }

  if (giveaway.status === "ended") {
    await interaction.editReply("❌ Ce giveaway est déjà terminé.");
    return;
  }
...
client.on("messageReactionAdd", async (reaction: MessageReaction | PartialMessageReaction, user: User | PartialUser) => {
  ...
  try {
    ...
    console.log(`✅ ${(user as User).username} a participé au giveaway #${giveaway.id}`);
  } catch (err) {
